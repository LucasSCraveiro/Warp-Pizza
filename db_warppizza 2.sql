...
--
-- Host: 127.0.0.1
-- Tempo de geração: 28-Nov-2023 às 02:42
...
CREATE TABLE `tb_endereco_usuario` (
  ...
  `nm_numero_logradouro` varchar(5) NOT NULL,
...
INSERT INTO `tb_endereco_usuario` (`cd_endereco_usuario`, `nm_logradouro`, `nm_numero_logradouro`, `nm_bairro`, `nm_cidade`, `sg_UF`, `cd_CEP`, `cd_usuario`) VALUES
(1, 'Rua Armando Vitório Bei', '76', 'Parque São Vicente', 'São Vicente', 'SP', '11365030', 1),
(2, 'Rua Armando Vitório Bei', '76', 'Parque São Vicente', 'São Vicente', 'SP', '11365030', 2);
...
CREATE TABLE `tb_pedido` (
  ...
  `nm_items_pedido` varchar(5000) NOT NULL,
  `vl_pedido` decimal(10,2) NOT NULL,
  `nm_endereco_pedido` varchar(200) NOT NULL,
  `cd_usuario` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `tb_usuario` (`cd_usuario`, `nm_usuario`, `dt_nascimento_usuario`, `nm_email_usuario`, `nm_senha_usuario`, `ds_tipo_usuario`) VALUES
(1, 'Lucas Craveiro', '2002-02-15', 'lucas.admin@email.com', '123', 'funcionario'),
(2, 'Lucas Craveiro', '2002-02-15', 'lucas.cliente@email.com', '123', 'cliente');
...
ALTER TABLE `tb_endereco_usuario`
  ...
  ADD KEY `fk_endereco_usuario` (`cd_usuario`);
...
--
ALTER TABLE `tb_pedido`
  ADD CONSTRAINT `fk_pedido_usuario` FOREIGN KEY (`cd_usuario`) REFERENCES `tb_usuario` (`cd_usuario`);
COMMIT;
